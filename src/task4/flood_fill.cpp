 ...
 *
 * * running command:
 * g++ ./flood_fill.cpp -o flood_fill -mwindows && ./flood_fill.exe
 *
...
void FloodFill(HDC hdc, int x, int y, COLORREF back_color, COLORREF fill_color)
{
  std::queue<Point> S;
  S.push(Point(x, y));
  while (!S.empty())
  {
    ...
    if (c != back_color)
      continue;
    SetPixel(hdc, v.x, v.y, fill_color);
...
void fill(HDC hdc, int x, int y, COLORREF fill_color){
  COLORREF c = GetPixel(hdc, x, y);
  FloodFill(hdc, x, y, c, fill_color);
}
...
LRESULT WndProc(HWND hwnd, UINT m, WPARAM wp, LPARAM lp)
{
  ...
  case WM_RBUTTONDOWN:
    ...
    fill(hdc, LOWORD(lp), HIWORD(lp), RGB(255, 0, 0));
    ReleaseDC(hwnd, hdc);
    break;
  case WM_CLOSE:
    DestroyWindow(hwnd);
